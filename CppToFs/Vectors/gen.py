...
if os.path.exists(args.path+"compiledVector"):
    os.remove(args.path+"compiledVector")

if os.path.exists(args.path+"compiledVector.exe"):
    os.remove(args.path+"compiledVector.exe")


# compiling the related C++ file
def compile_cpp(file_path):
    if platform == "darwin":  # macOs
        subprocess.run(["g++", "-o", "compiledVector", file_path])
    elif platform == "win32":  # Windows
        subprocess.run(
            ["g++", "-shared", "-o", "compiledVector.exe", "-m64", file_path])


compile_cpp("../../CppToFs/Vectors/Vector2.cpp")
...
if platform == "darwin":
    ...
    if os.path.exists(args.path+"FSHeader2D.fs"):
        ...
        imports2D = ["module vector2\n",
                     ...
                     '[<DllImport("compiledVector.exe", CallingConvention = CallingConvention.StdCall)>]\n',
