...
class SharedPtrProxyFeature:
    ...
    def init_type_converter(self, gen, conv):
        ...
        gen.add_cast(conv, self.wrapped_conv, lambda in_var,
                     out_var: '%s = ((%s *)%s)->get();\n' % (out_var, conv.ctype, in_var))
        conv._inline = True  # use inline alloc where possible
        # supports deep comparison by default (compare wrapped (T*)'s and not (std::shared_ptr<T>*)'s)
        conv._supports_deep_compare = True
...
def bind_future_T(gen, T, bound_name=None):
    ...
    gen.bind_named_enum('std::future_status', [
                        'deferred', 'ready', 'timeout'], prefix='future_')

    future = gen.begin_class(
        'std::future<%s>' % T, bound_name=bound_name, noncopyable=True, moveable=True)
