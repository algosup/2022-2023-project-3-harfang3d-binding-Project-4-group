...
parser.add_argument('--pybase', dest='python_base_path',
                    help='Path to the Python interpreter')
parser.add_argument('--luabase', dest='lua_base_path',
                    help='Path to the Lua interpreter')
parser.add_argument('--fsharpbase', dest='fsharp_base_path',
                    help='Path to the fsharp interpreter')
parser.add_argument('--go', dest='go_build',
                    help='Build GO', action="store_true")
parser.add_argument('--debug', dest='debug_test',
                    help='Generate a working solution to debug a test')
parser.add_argument('--x64', dest='x64', help='Build for 64 bit architecture',
                    action='store_true', default=False)
parser.add_argument('--linux', dest='linux',
                    help='Build on Linux', action='store_true', default=False)
...
def run_tests(gen, names, testbed):
    ...
    for i, name in enumerate(names):
        print('[%d/%d] Running test "%s" (%s)' %
              (i+1, test_count, name, gen.get_language()))
        cwd = os.getcwd()
    ...
    print("[Test summary: %d run, %d failed]" %
          (run_test_count, failed_test_count))
    ...
    def build_and_test_extension(self, work_path, module, sources):
        ...
        if args.linux:
            ...
            cflags = subprocess.check_output(
                'python3-config --cflags', shell=True).decode('utf-8').strip()
            cflags = cflags.replace('\n', ' ')

            build_cmd = 'gcc ' + cflags + \
                ' -g -O0 -fPIC -std=c++14 -c %s -o my_test.o' % ' '.join(
                    sources)

            try:
                subprocess.check_output(
                    build_cmd, shell=True, stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as e:
                print("Build error: ", e.output)
                return False

            ldflags = subprocess.check_output(
                'python3-config --ldflags', shell=True).decode('utf-8').strip()
            ...
            try:
                subprocess.check_output(
                    link_cmd, shell=True, stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as e:
                print("Link error: ", e.output)
                return False
            ...
            create_cpython_cmake_file(
                "test", work_path, sources, python_site_package, python_include_dir, python_library)
        ...
        success = True
        try:
            subprocess.check_output('%s -m test' %
                                    python_interpreter, shell=True)
        except subprocess.CalledProcessError as e:
            print(e.output)
            success = False
...
def build_and_deploy_lua_extension(work_path, build_path):
    ...
    shutil.copyfile(args.lua_base_path + '/bin/Debug/lua.exe',
                    os.path.join(work_path, 'lua.exe'))
    shutil.copyfile(args.lua_base_path + '/bin/Debug/lua53.dll',
                    os.path.join(work_path, 'lua53.dll'))
    ...
    def build_and_test_extension(self, work_path, module, sources):
        ...
        if args.linux:
            ...
            shutil.copy(os.path.join(
                args.lua_base_path, 'bin', 'lua'), work_path)

            build_cmd = 'gcc -I' + os.path.join(args.lua_base_path, 'include') + \
                ' -g -O0 -fPIC -std=c++14 -c %s -o my_test.o' % ' '.join(
                    sources)
            ...
            link_cmd = 'g++ -shared my_test.o -L' + \
                os.path.join(args.lua_base_path, 'lib') + \
                ' -o my_test.so -pthread'
            ...
            create_lua_cmake_file(
                "test", work_path, sources, args.lua_base_path)
            create_clang_format_file(work_path)
        ...
        try:
            subprocess.check_output(
                lua_interpreter + ' test.lua', shell=True, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            print(e.output)
            success = False
...
def create_go_cmake_file(module, work_path, sources):
    ...
    with open(cmake_path, 'w') as file:
        quoted_sources = ['"%s"' %
                          source for source in sources if ".go" not in source]
...
class GoTestBed:
    def build_and_test_extension(self, work_path, module, sources):
        ...
        try:
            subprocess.check_output(
                'go mod init mytest', shell=True, stderr=subprocess.STDOUT)
            subprocess.check_output(
                "go fmt mytest", shell=True, stderr=subprocess.STDOUT)
            subprocess.check_output(
                "goimports -w bind.go", shell=True, stderr=subprocess.STDOUT)
            subprocess.check_output(
                'go test -run ""', shell=True, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            print(e.output)
            success = False
...
class FsharpTestBed:
    def build_and_test_extension(self, work_path, module, sources):
        ...
        # copy test file
        test_path = os.path.join(work_path, 'test.fsx')
        with open(test_path, 'w') as file:
            file.write(module.fsharp_go)

        # if need special other file in package
        if hasattr(module, "test_special_cfsharp"):
            test_path = os.path.join(work_path, 'test_cfsharp.fsx')
            with open(test_path, 'w') as file:
                file.write(module.test_special_cfsharp)
        ...
        try:
            subprocess.check_output(
                'cd mytest', shell=True, stderr=subprocess.STDOUT)
            subprocess.check_output(
                'dotnet test ', shell=True, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            print(e.output)
            success = False
